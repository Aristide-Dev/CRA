...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $projectStatuses = Project::select('status')
        ...
        $projectsList = Project::select('id', 'name')->get();

        // Statistiques des projets filtrées
        $projectStats = Project::with(['activities' => function ($query) use ($startDate, $endDate) {
        ...
        $craStats = [
            ...
            'total_hours' => Activity::sum('duration'),
            'total_activities' => Activity::count(),
            'average_hours_per_day' => Activity::avg('duration'),
            ...
            'average_activities_per_cra' => round(Activity::count() / max(Cra::count(), 1), 2),
        ...
        $activitiesByType = Activity::select('type')
        ...
        $topProjectsActive = Project::withCount('activities')
        ...
        $topProjectsByHours = Project::with('activities')
            ->get()
            ->map(function ($project) {
                return [
                    'project' => [
